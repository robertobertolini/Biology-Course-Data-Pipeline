# Data pipeline incorporating the correlation attribute evaluation feature seletion technique during
...
of="Week9_Both_Correlation_Significant_GLMNET.xlsx"
of2="Week9_Both_Correlation_Significant_GLMNET_Score.xlsx"
...
for (k in 1:10){
  ...
  training_fail <- subset(listofdfs_train[[k]],y=='F')
  training_pass <- subset(listofdfs_train[[k]],y=='P')
  
  failing_count <- nrow(subset(listofdfs_train[[k]],y=='F'))
  passing_count <- nrow(subset(listofdfs_train[[k]],y=='P'))
  
  failing_mean <- sapply(training_fail[1:length(listofdfs_train[[k]])-1], mean)
  failing_var <- sapply(training_fail[1:length(listofdfs_train[[k]])-1], var)
  
  passing_mean <- sapply(training_pass[1:length(listofdfs_train[[k]])-1], mean)
  passing_var <- sapply(training_pass[1:length(listofdfs_train[[k]])-1], var)
  
  mu <- sapply(listofdfs_train[[k]][1:length(listofdfs_train[[k]])-1],mean)
  
  numerator = failing_count*(failing_mean-mu)^2 + passing_count*(passing_mean-mu)^2
  denominator = (failing_count-1)*failing_var + (passing_count-1)*passing_var
  
  fisher = as.data.frame(numerator/denominator)
  
  # End Fisher
  imp <- data.frame(overall = fisher,
                    names   = rownames(fisher))
  imp <- imp[order(abs(imp$numerator.denominator),decreasing = T),]
  
  imp_all <- imp[1:nrow(imp),]
  
  # Indicate ranking system - log_2(m)
  imp_6 <- imp[1:7,]
  
  # Now, do a modified ranking system
...
write.csv(auc_test,'AUC_GLMNET_Week9_Both_Correlation.csv')
write.csv(accu,'ACCURACY_GLMNET_Week9_Both_Correlation.csv')
write.csv(sensit,'SENSIT_GLMNET_Week9_Both_Correlation.csv')
write.csv(specif,'SPECIF_GLMNET_Week9_Both_Correlation.csv')
write.csv(bal_acc,'BALACC_GLMNET_Week9_Both_Correlation.csv')
write.csv(cut_point,'CP_GLMNET_Week9_Both_Correlation.csv')
write.csv(youden_j,'YOUDEN_GLMNET_Week9_Both_Correlation.csv')
write.csv(f1,'F1_GLMNET_Week9_Both_Correlation.csv')
write.csv(tp,'TP_GLMNET_Week9_Both_Correlation.csv')
write.csv(tn,'TN_GLMNET_Week9_Both_Correlation.csv')
write.csv(fn,'FN_GLMNET_Week9_Both_Correlation.csv')
write.csv(fp,'FP_GLMNET_Week9_Both_Correlation.csv')